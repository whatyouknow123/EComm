...
class Converter(object):
  ...
  def do(self):
    ...
    print ("convert data: %s" % self.input_path)
    for line in r:
      data = json.loads(line)
      self.parse_block(data, self.out)
    self.close()

  def parse_block(self, data, out):
    # node info
    node_info_bytes = 0
...
def usage():
  print('usage: ./json2date.py -c [input meta data file] -i [input file name] -o [output file name]')
